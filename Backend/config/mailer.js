...
const autoPreferSmtp = !emailProvider && !isProd && dmarcSensitiveFromDomain && hasSmtpCreds;
...
if (wantSendgrid) {
  ...
  const transporter = {
    ...
    sendMail: async (opts) => {
      ...
      const fromDomainInner = from.email.includes('@') ? from.email.split('@').pop().toLowerCase() : '';
      if (fromDomainInner === 'gmail.com' || fromDomainInner === 'googlemail.com') {
        throw new Error('SendGrid will likely fail delivery with a gmail.com From. Use a verified domain sender in EMAIL_FROM or switch EMAIL_PROVIDER=smtp.');
      }
  ...
  if (!emailUser || !emailPass) {
    module.exports = {
      provider: 'smtp',
      sendMail: async () => {
        throw new Error('SMTP credentials missing. Set EMAIL_USER and EMAIL_PASS or configure EMAIL_PROVIDER=sendgrid with SENDGRID_API_KEY.');
      },
